            ...
            <div className="flex">
                ...
                <div className="flex-1 p-6">
                    ...
                    <div className="grid grid-cols-1 gap-4 mb-8 lg:grid-cols-3">
                        <div className={`p-4 rounded-lg shadow-md ${darkMode ? 'bg-blue-500 text-white' : 'bg-blue-300 text-gray-900'}`}>
                            <h2 className="text-xl font-semibold">Total Records</h2>
                            <p className="mt-2 text-2xl">{data.length}</p>
                        </div>
                        <div className={`p-4 rounded-lg shadow-md ${darkMode ? 'bg-green-500 text-white' : 'bg-green-300 text-gray-900'}`}>
                            <h2 className="text-xl font-semibold">Unique Topics</h2>
                            <p className="mt-2 text-2xl">{topics.length}</p>
                        </div>
                        <div className={`p-4 rounded-lg shadow-md ${darkMode ? 'bg-yellow-500 text-white' : 'bg-yellow-300 text-gray-900'}`}>
                            <h2 className="text-xl font-semibold">Unique Sectors</h2>
                            <p className="mt-2 text-2xl">{sectors.length}</p>
                        </div>
                    </div>
                    <FilterComponent setFilters={setFilters} darkMode={darkMode} topics={topics} sectors={sectors} years={years} />
                    {loading ? (
                        ...
                        <>
                            <div className="flex justify-center mb-4 space-x-4">
                                <button className={`px-4 py-2 rounded ${activeChart === 'intensity' ? 'bg-blue-500 text-white' : 'bg-gray-300 dark:bg-gray-700 dark:text-white'} transition-colors duration-300`} onClick={() => setActiveChart('intensity')}>
                                    Intensity
                                </button>
                                <button className={`px-4 py-2 rounded ${activeChart === 'likelihood' ? 'bg-blue-500 text-white' : 'bg-gray-300 dark:bg-gray-700 dark:text-white'} transition-colors duration-300`} onClick={() => setActiveChart('likelihood')}>
                                    Likelihood
                                </button>
                                <button className={`px-4 py-2 rounded ${activeChart === 'relevance' ? 'bg-blue-500 text-white' : 'bg-gray-300 dark:bg-gray-700 dark:text-white'} transition-colors duration-300`} onClick={() => setActiveChart('relevance')}>
                                    Relevance
                                </button>
                                <button className={`px-4 py-2 rounded ${activeChart === 'trends' ? 'bg-blue-500 text-white' : 'bg-gray-300 dark:bg-gray-700 dark:text-white'} transition-colors duration-300`} onClick={() => setActiveChart('trends')}>
                                    Trends
                                </button>
                            </div>
                            {renderChart()}
