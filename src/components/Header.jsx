    ...
    <header className={`flex items-center justify-between px-4 py-3 border-b ${darkMode ? 'bg-gray-800 border-gray-700 text-white' : 'bg-white border-gray-200 text-gray-900'}`}>
      ...
      <div className={`relative flex-1 ${showSearchBar || window.innerWidth >= 768 ? 'block' : 'hidden'}`}>
        ...
        <input
          ...
          placeholder="Search..."
          className={`w-full px-4 py-2 rounded-full focus:outline-none transition-all duration-300 ${darkMode ? 'bg-gray-700 text-white' : 'bg-gray-100 text-gray-900'}`}
        />
        {showSearchBar && (
          <FaSearch className={`absolute right-3 top-1/2 transform -translate-y-1/2 text-lg ${darkMode ? 'text-gray-400' : 'text-gray-600'}`} />
        )}
      </div>
      ...
      <div className="flex items-center ml-4 space-x-4">
        ...
        <div className="relative" ref={profileMenuRef}>
          ...
          >
            <img
              src={profilePicURL || "https://img.freepik.com/free-photo/3d-cartoon-character_23-2151021986.jpg"}
